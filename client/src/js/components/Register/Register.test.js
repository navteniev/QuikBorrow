...
describe('Register', () => {
	...
	test('username entry', () => {
		...
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'name', value: 'test'}});
		expect(wrapper.state('name')).toEqual('test');
	});

	test('email entry', () => {
		let wrapper = shallow(<Register registerUser = {params.registerUser} auth = {params.auth} errors = {params.errors} />);
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'email', value: 'test@test.com'}});
		expect(wrapper.state('email')).toEqual('test@test.com');
	});

	test('password entry', () => {
		let wrapper = shallow(<Register registerUser = {params.registerUser} auth = {params.auth} errors = {params.errors} />);
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'password', value: 'password'}});
		expect(wrapper.state('password')).toEqual('password');
	});

	test('password confirmation entry', () => {
		let wrapper = shallow(<Register registerUser = {params.registerUser} auth = {params.auth} errors = {params.errors} />);
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'password2', value: 'password'}});
	...
	test('successful registration', () => {
		let wrapper = shallow(<Register registerUser = {params.registerUser} auth = {params.auth} errors = {params.errors} />);
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'name', value: 'test'}});
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'email', value: 'email@email.com'}});
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'password', value: 'password'}});
		wrapper.find('TextField[type="text"]').simulate('change', {target: {id: 'password2', value: 'password'}});
